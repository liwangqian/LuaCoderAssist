...
local function valid_id(id)
    return (Is.Number(id) or Is.String(id)), 'Invalid Event Id, Must be string/int/defines.events, Passed in: ' ..
        type(id)
end

local function valid_event_id(id)
    return (tonumber(id) and id >= 0) or
        (Is.String(id) and not bootstrap_register[id])
end
...
function Event.register(event_id, handler, matcher, pattern)
    ...
    Is.Assert(
        Is.Function(handler),
        'handler function is missing, use Event.remove to un register events'
    )
    Is.Assert(
        Is.Nil(matcher) or Is.Function(matcher),
        'matcher must be a function when present'
    )
    ...
    if #registry > 0 then
        for i, registered in ipairs(registry) do
            if
                registered.handler == handler and registered.pattern == pattern and
                    registered.matcher == matcher
             then
                table.remove(registry, i)
                log(
                    'Same handler already registered for event ' ..
                        event_id ..
                            ' at position ' .. i .. ', moving it to the bottom'
                )
                break
            end
        end
    ...
    table.insert(
        registry,
        {handler = handler, matcher = matcher, pattern = pattern}
    )
    return Event
end
...
function Event.remove(event_id, handler, matcher, pattern)
    ...
    if registry then
        ...
        elseif not found_something then
            log(
                'Attempt to deregister already non-registered listener from event: ' ..
                    event_id
            )
        end
    else
...
local function run_protected(registered, event)
    ...
    if Event.inspect_event or event.inspect_event then
        if game then
            ...
            game.write_file(
                script.mod_name .. '/event-trigger.lua',
                inspect(event) .. '\n',
...
function Event.get_event_handler(event_id)
    Is.Assert(valid_id(event_id))
    return {
        script = bootstrap_register(event_id) or
            (valid_event_id(event_id) and script.get_event_handler(event_id)),
