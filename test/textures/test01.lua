...
function Data:Flags()
    if self:valid() and self.flags then
        return setmetatable(
            self.flags,
            require('stdlib/utils/classes/string_array')
        )
    end
end
...
function Data:get(object, object_type, opts)
    ...
    if type(object) == 'table' then
        ...
        new = object
        local existing =
            data.raw[object.type] and data.raw[object.type][object.name]
    ...
    elseif type(object) == 'string' then
        ...
        local types =
            (object_type and {object_type}) or
            (self._class == 'Item' and item_and_fluid_types)
        if types then
            ...
            error(
                'object_type is missing for ' ..
                    (self._class or 'Unknown') .. '/' .. (object or ''),
                3
            )
        end
    end
    ...
    if new then
        ...
        msg =
            msg ..
            (object_type and (object_type .. '/') or '') ..
                tostring(object) .. ' does not exist.'
        ...
        trace =
            trace:gsub('%_%_.*%_%_%/stdlib/data.*\n', ''):gsub('\n', '->'):gsub(
            '\t',
            ''
        )
        trace = msg .. '  [' .. trace .. ']'
