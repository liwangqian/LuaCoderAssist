...
exports.parse = (walker, node, container, scope, parentSymbol, isDef) => {
    ...
    if (name === 'module') {
        parseModule(walker, node, container, scope, parentSymbol);
    } else if (name === 'require') {
        parseDependence(walker, node, container, scope, parentSymbol);
    } else if ((name == 'pcall') && (node.arguments[0].value == 'require')) {
        parseDependencePcall(walker, node, container, scope, parentSymbol);
    }
...
function parseModule(walker, node, container, scope, parentSymbol) {
...
function parseDependence(walker, node, container, scope, parentSymbol) {
    let moduleNode = node.argument || node.arguments[0];
    if (!moduleNode.value) {
        return;
    }
...
function parseDependencePcall(walker, node, container, scope, parentSymbol) {
